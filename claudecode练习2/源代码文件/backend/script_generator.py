...
# ── API 配置 ──────────────────────────────────────────
KIMI_KEY = ""  # 密钥已移除（请改用环境变量 / backend/.env）
KIMI_BASE = "https://api.moonshot.cn/v1"
KIMI_MODEL = "kimi-k2.7-code"

DEEPSEEK_KEY = ""  # 密钥已移除（请改用环境变量 / backend/.env）
...
async def generate_script(text: str) -> list[dict]:
    """
    双模型协作生成教学脚本。
    1. Kimi 通读文档 → 拆分场景
...
async def generate_animation_html(scenes: list[dict], mode: str = "3d") -> str:
    """
...
LESSON_SYSTEM_PROMPT = (
    ...
    "- steps 必须是恰好 3 个步骤（对应 检测→识别→响应 三个阶段）\n"
)
...
async def generate_lesson_json(scenes: list[dict], original_text: str = "", mode: str = "3d") -> dict:
    """
    ...
    resp = await _retry_on_rate_limit(_call, label)
    msg = resp.choices[0].message
    finish = resp.choices[0].finish_reason
    raw = (msg.content or "").strip()

    print(f"[{label}] 生成完成 — {len(raw)} 字符, finish_reason={finish}")
    if not raw:
        # 打印完整响应对象帮助调试
        import pprint
        print(f"[{label}] ⚠ 响应为空! 完整响应: ")
        pprint.pprint({
            "id": resp.id,
            "model": resp.model,
    ...
    try:
        ...
        if m:
            ...
            if start != -1 and end != -1 and end > start:
                ...
                raise ValueError(f"无法解析 lesson JSON，原始输出: {raw[:500]}...")
    ...
    # 确保 scene.type 合法
    valid_3d_types = ("lidar_obstacle_avoidance", "generic_3d")
    valid_2d_types = ("lidar_obstacle_avoidance_2d", "generic_2d")
    if mode == "2d":
...
async def generate_lesson_json_2d(scenes: list[dict], original_text: str = "") -> dict:
    """
    ...
    if original_text:
        ...
        user_prompt = (
            f"=== 文档内容（参考） ===\n{text_summary}\n\n"
            f"=== 教学场景 ===\n{json.dumps(scenes, ensure_ascii=False, indent=2)}\n\n"
            "请生成 2D 俯视视角的结构化 lesson.json。使用 *_2d 场景类型。"
        )
    else:
        user_prompt = (
            f"=== 教学场景 ===\n{json.dumps(scenes, ensure_ascii=False, indent=2)}\n\n"
