...
async def api_generate(file: UploadFile = File(...), mode: str = "3d"):
    ...
    try:
        ...
        # ── 2. 双模型生成场景 ──
        lesson_ir = await generate_script(text)
        scenes = lesson_ir.model_dump()["scenes"]

        # ── 3. 模板优先：尝试 lesson JSON，失败则降级 AI HTML ──
        response_mode = mode  # 保持原始 mode
        try:
            if mode == "2d":
                lesson_json = await generate_lesson_json_2d(lesson_ir, original_text=text)
            else:
                lesson_json = await generate_lesson_json(lesson_ir, original_text=text)
            ...
            animation_html = await generate_animation_html(lesson_ir, mode=mode)
            return JSONResponse(content={
                "scenes": scenes,
