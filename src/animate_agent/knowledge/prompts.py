...
- scenes 的数量由你根据文档内容复杂度自主决定：简单内容 3~5 个，
  一般 6~9 个，复杂内容 10~12 个；按教学顺序组织、从易到难，宁少勿滥。
- 每个场景必须覆盖一个「完整的语义单元」——一个概念、一个步骤或一条注意事项。
  若某个候选场景只能覆盖零散的半句内容，请与相邻场景合并，
  不要把一步拆成多个内容很薄的场景。
- **全片的 narration 加起来最多 350 字。** 这是一分钟的科普视频，画面上的字幕
  按每秒 6 个字推进，60 秒装不下更多。所以每个场景的 narration 写 20~60 字，
  口语化、面向初学者，只讲这一场景最核心的一件事。
  宁可多分几幕、每幕讲一句，也不要一幕里讲三句——幕多不占时间，字多才占。
- **narration 是要被念出来的话，不是讲义。** 没有「首先/其次/综上所述」这类
  书面连接词，没有括号里的补充说明。一句话讲完一个意思就断句。
